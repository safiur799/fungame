          ...
          <tbody>
            {visibleRows.map((user) => (
              <tr key={user.id} className="border-t border-white/10">
                <td className="py-3 font-black">{user.username}</td>
                <td className="py-3">{ROLE_LABELS[user.role]}</td>
                <td className="py-3">{user.createdByUsername || "-"}</td>
...
export function AdminPanel({ initialUser }: { initialUser: SessionUser | null }) {
  ...
  async function loadUserHistory(user: SessionUser) {
    setLoading(true);
    setMessage("");
    const response = await fetch(`/api/admin/user-history?userId=${encodeURIComponent(user.id)}`, { cache: "no-store" });
    const data = await response.json();
    setLoading(false);
    if (!response.ok) {
      setMessage(data.error || "Unable to load user history");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <form onSubmit={createAccount} className="rounded-xl border border-neon/20 bg-panel/80 p-5">
        ...
        {canSeePoints && (
          <form onSubmit={managePoints} className="rounded-xl border border-gold/20 bg-panel/80 p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_120px_120px_auto]">
              ...
              <input
                ...
                required
              />
              <button disabled={loading || !pointTargets.length} className="rounded-lg bg-hot px-4 py-3 font-black text-white disabled:opacity-60">
                Save
              </button>
            </div>
          </form>
              ...
              <tbody>
                {userHistory.history.map((item) => (
                  <tr key={item.roundId} className="border-t border-white/10">
                    <td className="py-3 font-mono">{item.roundId}</td>
                    <td className="py-3 font-mono">#{item.winningNumber}</td>
                    <td className="py-3">{item.numbers.join(", ")}</td>
              ...
              <div key={admin.adminId} className="rounded-lg border border-white/10 bg-ink/70 p-4">
                ...
                <div className="mt-3 overflow-x-auto">
                  <table className="w-full min-w-[760px] text-left text-xs">
                    <thead className="uppercase tracking-[0.14em] text-white/45">
                      <tr>
                    ...
                    <tbody>
                      {admin.users.map((user) => (
                        <tr key={user.userId} className="border-t border-white/10">
                          <td className="py-2 font-black">{user.username}</td>
        ...
        <section className="rounded-xl border border-white/10 bg-panel/80 p-5">
          ...
          <div className="mt-4 grid gap-2">
            {analytics.recentTransactions.slice(0, 30).map((transaction) => (
              <div key={transaction.id} className="flex flex-wrap justify-between gap-2 rounded-lg border border-white/10 bg-ink/70 p-3 text-sm">
                <span>
